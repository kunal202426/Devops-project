...
[![DevOps-Project-24: DotNet Monitoring](https://img.shields.io/badge/Project-DotNet%20Monitoring-brightgreen)]()
...
stage("Docker Build & tag"){
        ...
        stage("TRIVY"){
            steps{
                sh "trivy image dotnet-monitoring:latest > trivy.txt" 
            }
        }
        stage("Docker Push"){
            steps{
...
stage("Deploy to container"){
            steps{
                sh "docker run -d --name dotnet -p 5000:5000 dotnet-monitoring:latest"
            } 
        }
```
